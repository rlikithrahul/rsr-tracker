...
function getGenCode(p){
  return p.genCode || (p.documents && p.documents.gencode) || '—';
}

function getEANumber(p){
  return p.eaNumber || (p.documents && p.documents.ea) || '—';
}
  ...
  FIRMS.forEach(firm=>{
    ...
    const rows = projects.map((p,i)=>{
      ...
      return [
        ...
        (p.wecReceived||(p.documents&&p.documents.wec)||(typeof getWEXEntries==='function'&&getWEXEntries(p).length>0))?'Yes':'No'
      ];
    });
