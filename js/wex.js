...
const WEX_OVERRIDES_KEY = 'rsr_wex_label_overrides';

async function loadWEXOverrides(){
  if(D.wexOverrides) return D.wexOverrides;
  D.wexOverrides = await getSetting(WEX_OVERRIDES_KEY, {});
  return D.wexOverrides;
}
async function saveWEXOverrides(){
  await saveSetting(WEX_OVERRIDES_KEY, D.wexOverrides||{});
}
...
function getAllWEXItems(){
  const overrides=D.wexOverrides||{};
  const base=WEX_ITEMS.map(i=>overrides[i.key]?{...i,...overrides[i.key]}:i);
...
async function _editWEXCustomTypeFromSettings(idx){
  const ct = D.wexCustomTypes && D.wexCustomTypes[idx]; if(!ct) return;
  const newLabel = prompt('Rename this quantity type:', ct.label);
  if(newLabel===null) return;
  const newUnit = prompt('Unit:', ct.unit);
  if(newUnit===null) return;
  const backup = {...ct};
  ct.label = newLabel.trim()||ct.label;
  ct.unit = newUnit.trim()||ct.unit;
  try{
    await saveWEXCustomTypes();
    const el=document.getElementById('wex-types-settings-list');
    if(el) el.innerHTML=renderWEXTypesSettingsList();
